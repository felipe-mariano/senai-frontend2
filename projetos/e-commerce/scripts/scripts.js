...
let itensLançamento = document.querySelector(".itens-lancamento");
let id = 0;
for (const p of produtos) {
    ...
    <div class="">
        ...
        <div class="info">
            <h3>${p.nome}</h3>
            <h4>unidade<span>R$${p.sapato}</span></h4>
    ...
    for (const p of produtos) {
        if (p.quantidade > 0) {
            tbody.innerHTML += `
                        <tr>
                        <td>${p.nome}</td>
                        <td>${p.quantidade}x${p.velasAromaticas}=${p.quantidade*p.velasAromaticas}(unidade)</td>
                        <td>
                            <i class="bi bi-plus-square-fill" id="plus${id}"></i>
                            <i class="bi bi-dash-square-fill" id="dash${id}"></i>
                        </td>
                    </tr>`;
        total += p.quantidade*p.velasAromaticas;
...
enviar.addEventListener("click", ()=>{
    ...
    let total = 0;
       for (const p of produtos) {
        if (p.quantidade > 0) {
            msg += `${p.nome} ${p.quantidade}x${p.velasAromaticas}=${p.quantidade*p.velasAromaticas}\n`;
        total += p.quantidade * p.velasAromaticas;
